    ...
    <div className="relative min-h-screen w-full overflow-x-hidden bg-[#050505] text-white">
      ...
      <main className="mx-auto flex w-full max-w-[1200px] flex-col items-center gap-8 px-4 pb-16 pt-[clamp(100px,14vh,140px)] md:px-8">
        ...
        <div className="flex w-full max-w-[430px] flex-col items-center gap-4">
          <div className="flex w-full rounded-xl border border-gold/30 bg-black/30 p-1 shadow-[0_0_18px_rgba(212,175,55,0.12)] backdrop-blur-sm">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                type="button"
                ...
                className={`flex-1 rounded-lg px-4 py-3 text-sm font-semibold uppercase tracking-[0.15em] transition-all duration-300 ${
                  activeTab === tab.key
                    ? 'bg-gradient-to-r from-[#b78000] via-[#ffdb86] to-[#d4af37] text-black shadow-[0_0_18px_rgba(212,175,55,0.45)]'
                    : 'text-gold/80 hover:bg-gold/5 hover:text-gold'
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="flex w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/3 px-4 py-3 text-sm text-white/80">
            ...
            <select
              ...
              className="rounded-lg border border-gold/30 bg-[#0d0d0d] px-3 py-2 text-xs uppercase tracking-[0.12em] text-gold outline-none"
            >
          ...
          <div className="grid w-full max-w-[1100px] grid-cols-1 place-items-center gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {visibleEvents.map((event) => (
              <button
                ...
                className="group w-full max-w-[320px] rounded-2xl border border-white/10 bg-black/30 p-3 text-left transition-all duration-300 hover:-translate-y-1 hover:border-gold/40 hover:shadow-[0_0_28px_rgba(212,175,55,0.18)]"
              >
