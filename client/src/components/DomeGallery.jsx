...
export default function DomeGallery({
 ...
 // Rotation states: base drag rotation + scroll parallax rotation
 const rotationRef = useRef({ x: 0, y: 0 });
 const scrollOffsetRef = useRef({ x: 0, y: 0 });
 const dragBaseRotRef = useRef({ x: 0, y: 0 });
 ...
 const totalX = clamp(
 dragBaseRotRef.current.x + scrollOffsetRef.current.x,
 -maxVerticalRotationDeg,
 maxVerticalRotationDeg
 );
 const totalY = wrapAngleSigned(dragBaseRotRef.current.y + scrollOffsetRef.current.y);
 rotationRef.current = { x: totalX, y: totalY };
 applyTransform(totalX, totalY);
 ...
 // Scroll Parallax Effect with ScrollTrigger and fallback listener
 useEffect(() => {
 if (!scrollParallax || !rootRef.current) return;
 ...
 scrub: 1.2,
 onUpdate: (self) => {
 if (focusedElRef.current || rootRef.current?.getAttribute('data-enlarging') === 'true') return;
 const progress = self.progress;
 const rotY = (progress - 0.5) * scrollParallaxAngle;
 const rotX = Math.sin((progress - 0.5) * Math.PI) * (maxVerticalRotationDeg * 0.4);
 
 scrollOffsetRef.current = { x: rotX, y: rotY };
 ...
 }, [scrollParallax, scrollParallaxAngle, maxVerticalRotationDeg, updateCombinedRotation]);
 ...
 const nextX = clamp(dragBaseRotRef.current.x - vY / 200, -maxVerticalRotationDeg, maxVerticalRotationDeg);
 const nextY = wrapAngleSigned(dragBaseRotRef.current.y + vX / 200);
 dragBaseRotRef.current = { x: nextX, y: nextY };
 updateCombinedRotation();
 ...
 [dragDampening, maxVerticalRotationDeg, stopInertia, updateCombinedRotation]
 );
 ...
 startRotRef.current = { ...dragBaseRotRef.current };
 ...
 const cur = dragBaseRotRef.current;
